...
/**
 * KISA(한국인터넷진흥원)에서 배포한 SEED 블록 암호 알고리즘의 CBC(Cipher Block Chaining) 모드 구현체입니다.
 * <p>
 * 이 클래스는 SEED 암호화 및 복호화에 필요한 모든 저수준 연산과
 * 이를 편리하게 사용할 수 있는 고수준 API를 제공합니다.
 *
 * @author KISA (Copyright (c) 2013)
 * @see <a href="http://seed.kisa.or.kr/">http://seed.kisa.or.kr/</a>
 */
public class KISA_SEED_CBC {

	/**
	 * 기본 엔디안 설정 (자바는 기본적으로 Big-Endian)
	 */
	...
	/** 32비트 정수의 바이트 순서(엔디안)를 변경합니다. (Big-Endian <-> Little-Endian) */
	...
	/** 두 개의 128비트(정수 4개) 블록을 XOR 연산합니다. */
	...
	/**
	 * SEED-CBC 암/복호화 컨텍스트를 초기화합니다.
	 * 이 함수는 키 스케줄링을 수행하고 IV를 설정합니다.
	 * 
	 * @param pInfo       암/복호화 상태 정보를 저장할 컨텍스트 객체
	 * @param enc         암호화(ENCRYPT) 또는 복호화(DECRYPT) 모드
	 * @param pbszUserKey 16바이트 사용자 키
	 * @param pbszIV      16바이트 초기화 벡터
	 */
	...
	/**
	 * 여러 블록의 데이터를 순차적으로 암/복호화합니다.
	 * 
	 * @param pInfo  초기화된 암/복호화 컨텍스트 객체
	 * @param in     입력 데이터 (평문 또는 암호문)
	 * @param inLen  입력 데이터의 길이 (바이트 단위)
	 * @param out    출력 데이터가 저장될 배열
	 * @param outLen 처리된 출력 데이터의 길이를 반환받을 배열
	 */
	...
	/**
	 * 마지막 데이터 블록을 처리합니다.
	 * 암호화 시에는 PKCS#7 패딩을 추가하고, 복호화 시에는 패딩을 제거합니다.
	 * 
	 * @param pInfo  암/복호화 컨텍스트 객체
	 * @param out    마지막 블록 처리 결과가 저장될 배열
	 * @param outLen 마지막 블록 처리 결과의 길이를 반환받을 배열
	 */
	...
	/**
	 * SEED-CBC 모드로 암호화를 수행하는 고수준 편의 메서드입니다.
	 * 내부적으로 PKCS#7 패딩을 적용합니다.
	 *
	 * @param pbszUserKey    16바이트 사용자 키 (암호화에 사용될 비밀키)
	 * @param pbszIV         16바이트 초기화 벡터 (Initialization Vector)
	 * @param message        암호화할 평문 데이터 바이트 배열
	 * @param message_offset 평문 데이터의 시작 오프셋
	 * @param message_length 암호화할 평문의 길이
	 * @return 암호화된 데이터(Ciphertext) 바이트 배열
	 */
	...
	/**
	 * SEED-CBC 모드로 복호화를 수행하는 고수준 편의 메서드입니다.
	 * 내부적으로 PKCS#7 패딩을 제거합니다.
	 *
	 * @param pbszUserKey    16바이트 사용자 키 (복호화에 사용될 비밀키, 암호화 키와 동일해야 함)
	 * @param pbszIV         16바이트 초기화 벡터 (암호화 시 사용된 IV와 동일해야 함)
	 * @param message        복호화할 암호문 데이터 바이트 배열
	 * @param message_offset 암호문 데이터의 시작 오프셋
	 * @param message_length 복호화할 암호문의 길이
	 * @return 복호화된 데이터(Plaintext) 바이트 배열
	 */
	...
	/**
	 * SEED-CBC 암/복호화 과정의 상태 정보를 저장하는 컨텍스트 클래스
	 */
	...
	/**
	 * 저수준의 바이트/정수 변환, 배열 복사 등 공통 유틸리티 함수를 제공하는 내부 클래스
	 */
	public static class Common {

		/** 엔디안 상수: Big-Endian (네트워크 바이트 순서) */
		public static final int BIG_ENDIAN = 0;
		/** 엔디안 상수: Little-Endian */
